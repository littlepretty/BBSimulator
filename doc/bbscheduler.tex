...
%\textit{data stage-in} phase, \textit{running} phase, and \textit{data stage-out} phase.
%The novel 3-phase BB aware batch scheduler \textit{Cerberus}
%allocate resources in different phases separately.
%In both data stage input and output phase, it is possible to
%maximize data transfer throughput or task parallelism when allocating BB resources.
%In the running phase, we maximize the profit of scheduled tasks.
%We simulate Cerberus scheduling the Trinity supercomputing platform to demonstrate that
%1) burst buffer nodes can accelerate the execution pipeline of all tasks,
%thus improving the task response time
%2) our 3-phase model and dynamic programming solutions further
%benefit burst buffer equipped system.

Burst buffer nodes(BB) are drastically improving
the performance of computational application by providing high perceived IO bandwidth.
However, traditional batch scheduler either barely knows BB's existence
or cannot fully utilize them in practice.
In this paper, we model the execution of typical scientific applications
that generates tens of TBytes of data on super computer equipped with BB.
We characterize generic applications to 3 phases:
\textit{data stage-in}, \textit{running}, and \textit{data stage-out} phase.
...
between BB and external storage system is maximized.
In the running phase, it maximizes the profit of scheduled tasks.
Simulation results of Cerberus scheduling the
BB-enabled supercomputer platform demonstrates that
1) comparing to non-BB system, 
Cerberus accelerates the execution pipeline of all tasks by utilizing BB,
reducing the response time of 99\% jobs;
2) on BB-enabled system, 3-phase Cerberus and BB oriented optimization
together further improve average system throughput by 62.31\%.
\end{abstract}
