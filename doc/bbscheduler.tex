...
\begin{align*}
        & \max \sum_{i\in S} bb\_sin_i \textit{   s.t.}\\
        & \left\{
                \begin{array}{l}
...
\begin{align*}
        & \max \sum_{i \in S}c_i * bb_i \textit{   s.t.}\\
        & \left\{
                \begin{array}{l}
                        S \cup NS = J \\ [1em]
...
First, for problem \ref{Equ:MaxTransferData}, the recursive relationship is given by
...
\begin{align}
        ...
        & \left\{
                \begin{array}{l}
                        0, \textit{ if $i=0$ } \\ [1em]
                        dp(i-1, w), \textit{ if $bb\_sin_i > w$} \\ [1em]
                        \max \{ dp(i-1, w), dp(i-1, w-bb\_sin_i) + bb\_sin_i \}, \textif{ if $bb\_sin_i <= w$}
                \end{array} 
        \right.
...
\begin{align}
        ...
        & \left\{
                \begin{array}{l}
                        0, \textit{ if $i=0$ } \\ [1em]
                        dp(i-1, w), \textit{ if $bb\_sin_i > w$} \\ [1em]
                        \max \{ dp(i-1, w), dp(i-1, w-bb\_sin_i) + 1 \}, \textif{ if $bb\_sin_i <= w$}
                \end{array} 
        \right.
...
\begin{align}
        ...
        & \left\{
                \begin{array}{l}
                        0, \textit{ if $i=0$ } \\ [1em]
                        dp(i-1, c, w), \textit{ if $c_i > c$ or $bb\_sin_i > w$} \\ [1em]
                        \max \{ dp(i-1, c, w), dp(i-1, c - c_i, w-bb\_sin_i) + 1 \}, \textif{ if $c_i <= c$ and $bb\_sin_i <= w$}
                \end{array} 
        \right.
...
We start from $dp(n, CN, BB)$; if $c_n <= CN$ and $bb\_sin_n <= BB$, $job_n$ should be scheduled if
...
The time complexity of solving \ref{Equ:MaxTransferDataRecursion} and \ref{Equ:MaxTaskNumberRecursion} is $O(n*BB)$.
The time complexity of solving \ref{Equ:MaxProductRecursion} is $O(n*CN*BB)$.
