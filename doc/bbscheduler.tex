...
In this paper, we model the execution of applications on supercomputing system shipped with burst buffer nodes.
...
Resources in these phases are intuitively scheduled separately with possible different goals.
In both data stage I/O phase, it is possible to maximize data transfer throughput or task parallelism by allocating burst buffer.
In the running phase, ...
We simulate the scenarios both with and without burst buffer on a discrete event simulator to demonstrate its benefit to system performance.
\end{abstract}
...
Therefore, each job is associated with a demand vector in the form of $(c, bb_{sin}, bb)$,
where $c$ is the number of needed compute node in running phase,
$bb_{sin}$ is the volumn of burst buffer user predicted for file stage in,
$bb$ is the volumn of burst buffer user preferred.
A benign user should set $bb = \max\{bb_{running}, bb_{stage-out}\}$.
However, we do not make any assumption about $bb_{running}$ and $bb_{stage-out}$ because it is nontrivial to predict them,
...
If we care about data transfer throughput, we should do the following optimization:
\begin{align}
& \max \sum_{i\in S} bb[i]_{sin} \\
& S \cup NS = J \\
& \sum_{i\in S} bb[i]_{sin} = BB
\end{align}
...
Optimization for different purpose are the same as in IO-BB scheduling.
