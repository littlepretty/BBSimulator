...
lower boundary 1 TB and upper boundary 60 TB.
...
which is 4.11 times slow as the most non-responsive job
...
Without burst buffer nodes, job's wait duration in worst case is 6.1 times
slow as the worst one on systems with burst buffer;
the upper bound of waiting duration for burst buffer systems is about 200,000 seconds.
...
Let's compare scheduling results of 1-phase and 3-phase D, both of which only have rough data information of application.
More than 60\% of the 3-phase-modeled jobs finish faster than 1-phase-modeled jobs.
The longest 3-phase-modeled job takes 400,000 seconds to finish
while the slowest 1-phase-modeled job needs about 540,000 seconds to finish.
...
we see that jobs did not spend much time in either input queue or output queue.
...
Job \#439 waits 86442 seconds in the maximizing burst buffer usage case;
job \#463 waits 86998 seconds in the case of maximizing number of tasks.
...
large number of cores but request less running time.
...
favors the jobs with less request time.
...
for 80\% jobs, is not strategy-proofness\cite{Ghodsi:NSDI:2011}
