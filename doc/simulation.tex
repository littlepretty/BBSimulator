...
We develop an event-driven simulator for burst buffer enabled HPC system, named BBSim,
...
The longest 3-phase-modeled job takes 490,174 seconds to finish
while the slowest 1-phase-modeled job needs about 828,991 seconds to finish.
The improvement is about 40.87\% for the worst case.
...
the worst absolute response time is less than 391,163 seconds.
This is 20.20\% improvement to 3-phase-modeled jobs
when Cerberus only knows the upper bound of data demand,
52.81\% better than the slowest 1-phase-modeled job.
...
Meanwhile, \textit{more than 90\% of the jobs takes less time if user 
...
$<$ 12.5\% and $<$ 6\% of the total waiting time in the case of 1-phase scheduler
...
\textit{In average, the throughput of Cerberus is 1.518 jobs / 500 seconds.}
It is 112\% of the 1-Phase BB case (0.716 jobs / 500 seconds) and
25.35\% higher than the 1D Cerberus case (1.211 jobs / 500 seconds).
...
\begin{figure*}[!t]
        ...
        \subfloat[Job Wait Run Time] {
                \includegraphics[width=2.3in]{3Pvs1PFigures/1000jobs_3p_vs_1p_wait_run}
...
taking 359,134 and 352,873 seconds respectively.
In contrast, job \#445 takes 391,163 seconds to finish in FCFS Cerberus.
This is 8.19\% slower than MaxT Cerberus and 9.79\% slower than MaxP Cerberus.
...
Job \#435 waits 109,214 seconds in the maximizing burst buffer usage case;
job \#438 waits 102,935 seconds in the case of maximizing number of tasks.
...
but its expected running time is just 3600 seconds;
...
That is 359,606 seconds for MaxT Cerberus and 353,345 seconds for MaxP Cerberus.
FCFS Cerberus makes system ends its last job, also job \#455, at 391,635 seconds.
The worst case completion improvement to FCFS-style Cerberus is
8.18\% for MaxT Cerberus and 9.78\% for MaxP Cerberus.
When we look at the time sequence of throughput,
we found the peak value 11 jobs / 500 seconds obtained by MaxP Cerberus.
The alternative optimization method, MaxT Cerberus,
could achieve 15 jobs / 500 seconds.
The peak throughput of FCFS Cerberus is 12 jobs / 500 seconds around 50000 seconds.
...
9.62\% higher average throughput (1.664 jobs / 500 seconds)
while MaxP Cerberus gives 10.54\% improvement (1.678 jobs / 500 seconds)
that is, extra 0.92\% enhancement to MaxT Cerberus.}
...
\begin{table}[!t] 
        ...
        \begin{tabular}{l|c|c}
                ...
                \hline
                \hline
                Simulation Run Time / seconds & 246.87 & 257.42 \\
                Optimization Run Time / seconds & 241.11 & 251.75 \\
